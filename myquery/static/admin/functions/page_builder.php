...
function builder_meta_boxes() {
	global $post;
	add_meta_box ('builder_slideshow',__('Page slideshow','infocenter'),'builder_slideshow','page','normal','high');
	add_meta_box ('builder_slideshow',__('Page slideshow','infocenter'),'builder_slideshow','post','normal','high');
	add_meta_box ('sort_the_sections',__('Sort the sections','infocenter'),'sort_the_sections','post','side','low');
	add_meta_box ('sort_the_sections',__('Sort the sections','infocenter'),'sort_the_sections','question','side','low');
}
/*-----------------------------------------------------------------------------------*/
/* Sort the sections */
/*-----------------------------------------------------------------------------------*/
function sort_the_sections() {
	...
	$infocenter_custom_sections = get_post_meta($post->ID,"infocenter_custom_sections",true);
	?>
	<div class="minor-publishing">
		<div class="rwmb-field">
			<div class="rwmb-label">
				<label for="infocenter_custom_sections"><?php echo esc_html_e("Custom sections","infocenter");?></label>
			</div>
			<div class="rwmb-input vpanel_checkbox_input">
				<input type="checkbox" class="rwmb-checkbox" name="infocenter_custom_sections" id="infocenter_custom_sections" value="1"<?php if (isset($infocenter_custom_sections) && $infocenter_custom_sections == 1) {echo " checked='checked'";}?>>
			</div>
		</div>
		<ul id="sort-sections">
			<?php
...
function builder_slideshow() {
    ...
    <div id="builder_slide_warp">
		<div class="add-item" add-item="add_slide"><?php esc_html_e('+ Add new slide','infocenter')?></div>
	    <div class="clear"></div>
		<ul id="builder_slide">
	    	<?php
				...
				foreach ($builder_slide_item as $builder_slide) {$k++;
					...
					<li id="builder_slide_<?php echo esc_attr($k);?>" class="ui-state-default">
						<div class="widget-head">
							<span class="vpanel<?php echo esc_attr($k);?>"><?php echo esc_html_e("Slide item","infocenter");?> - <?php echo esc_attr($k);?></span>
						...
						<div class="widget-content">
						    <label for="builder_slide_item[<?php echo esc_attr($k);?>][image_url]">
						    	<span><?php echo esc_html_e("Image URL","infocenter");?> :</span>
						    ...
						    <label for="builder_slide_item[<?php echo esc_attr($k);?>][slide_link]">
						    	<span><?php echo esc_html_e("Slide Link","infocenter");?> :</span>
						...
						<a class="del-builder-item"><?php echo esc_html_e("x","infocenter");?></a>
					</li>
			<?php }
			}else {
				echo "";
...
function builder_meta_save ($post_id,$post) {
	...
	if (isset($_POST["infocenter_custom_sections"]) && $_POST["infocenter_custom_sections"] == 1) {
		$infocenter_custom_sections = $_POST["infocenter_custom_sections"];
		update_post_meta($post->ID,"infocenter_custom_sections",$infocenter_custom_sections);
	}else {
		delete_post_meta($post->ID,"infocenter_custom_sections");
	}
